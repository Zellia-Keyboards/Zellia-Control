<script lang="ts">
	...
	function updateConfiguration(): void {
		...
		updateGlobalConfiguration(
			keyId,
			config as GlobalDynamicKeystrokeConfiguration,
		);
	}
	...
	$effect(() => {
		if ($CurrentSelected) {
			...
			if (
				...
				selectedBitmaps = dynamicConfig.bitmap.map((b: DKSAction[]) => [
					...b,
				]);
	...
	function handleSelectBinding(bindingIdx: number) {
		selectedBindingIndex =
			selectedBindingIndex === bindingIdx ? null : bindingIdx;
	}
</script>
				...
				<div class="flex gap-8">
					<div class="w-96 flex flex-col gap-4">
							...
							<div class="space-y-2">
								{#each selectedKeycodes as keycode, bindingIndex (bindingIndex)}
									<DKSBindingRow
										{bindingIndex}
										{keycode}
										selectedBitmap={selectedBitmaps[
											bindingIndex
										]}
						...
						<BottomOutPointConfig
							bind:bottomOutPointValue={bottomOutPoint}
						/>
					</div>
