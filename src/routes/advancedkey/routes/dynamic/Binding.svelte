<script lang="ts">
    ...
    interface Props {
        ...
        actionCategories: { name: string; actions: { id: string; name: string }[] }[];
    }

    let { selectedKeycodes, selectedBindingIndex, actionCategories }: Props = $props();

    function selectKeycode(keycode: string): void {
        if (selectedBindingIndex !== null) {
            selectedKeycodes[selectedBindingIndex] = keycode; // Directly mutates the $state array, Svelte 5 tracks this [cite: 222, 223]
